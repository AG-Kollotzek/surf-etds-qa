...
Aufbau (8 Felder von oben nach unten):
    1-6  je ein Freiheitsgrad (Longitudinal, Lateral, Vertical, Roll, Pitch, Yaw).
         ...
         die duennen gestrichelten Linien die Mitten der beiden Sync-Pulse.
...
from qa_metrics import DOF_SPEC
...
def _draw_measurement_window(ax, t_win_start, t_win_end, color):
    """Grau schraffierter Hintergrund fuer den ausgewerteten Zeitraum."""
    ax.axvspan(t_win_start, t_win_end, facecolor='0.6', alpha=0.13,
               hatch='///', edgecolor=color, linewidth=0.0, zorder=0)
...
def plot_qa_overview(records, sd_table=None, title="ETDS QA Overview", save_path=None):
    ...
    try:
        ...
        # automatische Layout-Berechnung sonst verzerren.
        fig.subplots_adjust(left=0.115, right=0.975, top=0.955, bottom=0.025, hspace=0.55)
        ...
        for ax, dof in zip(axes[:6], DOF_SPEC):
            ...
            ax.legend(loc='upper left', fontsize=5.5, ncol=2, handlelength=1.8,
        ...
        for p in prepared:
            ...
            _draw_measurement_window(ax_rmse, p['win_start_rel'], p['win_end_rel'], p['color'])
        ...
        ax_rmse.legend(loc='upper left', fontsize=5.5, ncol=2, handlelength=1.8,
                       borderpad=0.3, labelspacing=0.25, columnspacing=0.9)
