...
\usepackage{fontspec}
\setmainfont{Arial}          % gleiche Schriftart wie die matplotlib-Plots (qa_plots.py)
\setsansfont{Arial}
...
\newcommand{\ContactName}{<<CONTACT_NAME>>}
\newcommand{\ContactMailOne}{<<CONTACT_MAIL_1>>}
\newcommand{\ContactMailTwo}{<<CONTACT_MAIL_2>>}

% Briefkopf-Block: steht im Detailreport rechts neben den Abkuerzungen.
% \href statt \url, damit die Adressen in der Dokumentschrift (Arial) statt in
% Schreibmaschinenschrift gesetzt werden; \raggedright verhindert, dass der Blocksatz
% das Trennzeichen zwischen den beiden Adressen ans Zeilenende schiebt.
\newcommand{\ContactBlock}{%
  \raggedright
  \textbf{Contact for data analysis and management:} \\
  \ContactName \\
  \href{mailto:<<CONTACT_MAIL_1>>}{\ContactMailOne} /
  \href{mailto:<<CONTACT_MAIL_2>>}{\ContactMailTwo} \\
  IGRT-Lab: \href{<<IGRT_LAB_URL>>}{<<IGRT_LAB_URL>>} \\
}
...
\fancyfoot[R]{\footnotesize \href{<<IGRT_LAB_URL>>}{<<IGRT_LAB_URL>>}\\ Page \thepage\ of \pageref{LastPage}}
...
\begin{tabular}[l]{@{}c@{}}
  {\bfseries\itshape }\\
  \includegraphics[height=1.75cm]{Logo/logo.jpg}
\end{tabular}
  \end{tabular}
  \end{center}
  \vspace{2mm}
...
\include{Report/overview}
\include{Report/detailedreport}
\include{Report/plots}
